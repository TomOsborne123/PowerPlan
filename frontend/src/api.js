...
export function usePostcodeLookup(setLatitude, setLongitude) {
  ...
  const lookup = useCallback(async (postcode) => {
    ...
    try {
      const r = await fetch(apiUrl('/api/postcode'), {
        method: 'POST',
...
export async function fetchScrapeResults(postcode) {
  const norm = normalizePostcode(postcode)
  if (!norm) return null
  const r = await fetch(apiUrl(`/api/scrape-results?postcode=${encodeURIComponent(norm)}`))
  const data = await r.json()
  if (!r.ok) return null
  return data
}
...
export async function fetchScrapeStatus(postcode) {
  ...
  const r = await fetch(apiUrl(`/api/scrape-status?postcode=${encodeURIComponent(norm)}`))
  const data = await r.json()
